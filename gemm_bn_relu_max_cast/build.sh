...
if [[ $# == 1 && -f $1 ]]; then
    ...
    ${cmd} gemm_bn_relu_max_cast.cu profiler.cc main.cc -o ${target_dir}/${target_file}
    # nvprof \
    # nsys profile --trace=cuda,nvtx --backtrace=none --sample=none -o "/home/administrator/workspace/tmp/`date +%F-%T`.qdrep" \
    run_cmd="${target_dir}/${target_file} -M=576000 -MM=18000 -N=64 -K=10 -verbose=true -doProfile=true -doRefCheck=true -runNum=1000"
    echo $run_cmd
    $run_cmd
fi
