...
if [[ $# == 1 && -f $1 ]]; then
    ...
    ${cmd} transpose2d.cu main.cc -o ${target_dir}/${target_file}
    # nvprof \
    # nsys profile --trace=cuda,nvtx --backtrace=none --sample=none -o "/home/administrator/workspace/tmp/`date +%F-%T`.qdrep" \
    # run_cmd="${target_dir}/${target_file} -N=128 -C=64 -verbose=true -random=false -doProfile=true -doRefCheck=true -runNum=1000"
    run_cmd="${target_dir}/${target_file} -N=1024 -C=1024 -verbose=true -random=false -doProfile=true -doRefCheck=true -runNum=1000"
    echo $run_cmd
    $run_cmd
fi
