...
__global__ void hgemm_tensorcore_128x128_doublebuffer_ldgsts_v3_kernel(half* __restrict__ A, half* __restrict__ B, half* __restrict__ C) {
  ...
  #undef bx
  int const bx = blockIdx.z * gridDim.x + blockIdx.x;
...
////////////////////////////////////////////////////////////////////////
// 7. hgemm tensorcore 128x128 double buffer ldgsts nsplit
// (Improve the hit rate of L2 Cache. It's proven useful on JETSON Orin, but not ideal on RTX 30 series)
// A: row_major [M, K]
// B: row_major [K, N]
// C: row_major [M, N]
template<int M, int N, int K,int BM, int BN, int BK>
