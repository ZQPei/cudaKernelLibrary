...
if [[ $# == 1 && -f $1 ]]; then
    ...
    ${cmd} batchnorm_relu_max.cu profiler.cc main.cc -o ${target_dir}/${target_file}
    # nvprof \
    # nsys profile --trace=cuda,nvtx --backtrace=none --sample=none -o "/home/administrator/workspace/tmp/`date +%F-%T`.qdrep" \
    run_cmd="${target_dir}/${target_file} -N=18000 -D=32 -C=64 -verbose=true -doProfile=true -doRefCheck=true -runNum=1000"
    echo $run_cmd
    $run_cmd
fi
